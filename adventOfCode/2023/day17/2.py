...
def solve(inp):
    ...
    min_heat_loss = [[[[10000 for _ in range(len(grid[0]))] for _ in range(len(grid))] for _ in range(11)] for _ in range(4)]
    ...
    for mhl in range(10*(len(grid) + len(grid[0]))):
        ...
        for ldr, trv, rr, cc in pq[mhl]:
            ...
            next_dirs = []
            if trv >= 4:
                next_dirs.extend(nxt_drs[ldr])
            if trv < 10:
                next_dirs.append(ldr)
